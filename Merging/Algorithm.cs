...
public static class Algorithm
{
    public static void Merge(TowerModel first, TowerModel? second)
    {
        ...
        first.range = (first.range + second.range) / 2;

        first.mods = first.mods.Union(second.mods).ToArray();

        try
        {
            var footprintInfo = Il2CppType.Of<TowerModel>().GetProperty("footprint");
            DeepMerge(footprintInfo, first, second, null);
        ...
        {
            MelonLogger.Error("Error during non-crucial merges");
            MelonLogger.Warning(e);
        }

        var behaviorsInfo = Il2CppType.Of<TowerModel>().GetField("behaviors");
        MergeField(behaviorsInfo, first, second, null);
        ...
        History history, bool shallow = false)
    {
        ...
        if (elementType.IsType<Model>())
        {
            ...
            foreach (var rightThing in rightStuff)
            {
                ...
                //todo: this is what will probably break
                else
                {
                    leftModel.AddChildDependant(rightModel);
                }
            }
        }
