...
public partial class Main
{
    ...
    private static void TowerSelectionMenu_Initialise(TowerSelectionMenu __instance)
    {
        ...
        cost = finalselect.AddText(new Info("MergeCost", buttonSize, buttonSize, new Vector2(.765f, .15f)), "", 45f);
        ...
        mergebutton =
            finalselect.AddButton(new Info("MergeButton", 350, 200, new Vector2(.765f, .5f)),
                VanillaSprites.GreenBtnLong, new Action(() =>
                {
                    ...
                    TowerSelectionMenu.instance.selectedTower.tower.SetTowerModel(
                        Algorithm.Merge(TowerSelectionMenu.instance.selectedTower.Def, selectedtower));
        ...
        mergetext = mergebutton.AddText(new Info("MergeText", buttonSize, buttonSize, new Vector2(.5f, .55f)), "Merge",
            65f);


        towerportrait = finalselect.AddImage(new Info("TowerPortrait", 300, 300, new Vector2(.2f, .5f)), "");

        invalidtext = finalselect.AddText(new Info("InvalidText", 500, 100, new Vector2(.5f, .875f)), "Invalid Tower",
            55f);
